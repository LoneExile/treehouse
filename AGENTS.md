...
- Resetting a worktree re-aligns nested submodules (`syncSubmodules` in `internal/git/git.go`), because `checkout`/`reset`/`clean` do not recurse into them and a stale submodule gitlink keeps `IsDirty` true forever, so the slot is never reissued and the pool starves. Three parts are all load-bearing: `-c protocol.file.allow=always` (a pooled worktree's submodule remote is a local path, and git refuses that "file" transport by default since CVE-2022-39253 - it MUST be a `-c` flag, since repo config does not reach the child clone/fetch process), `--force` (otherwise a modified submodule working tree stays dirty and update reports success), and a recursive `git clean -ffdq` inside submodules (untracked submodule content also marks the superproject dirty). Failures propagate: a reset that cannot align submodules must fail rather than return an unusable slot to the pool looking healthy
- A fourth `-c` flag on that same call is `fetch.recurseSubmodules=no` (see the `noFetchRecurseSubmodules` comment for the full trace). `submodule update --recursive` already descends level by level, so fetch-time recursion is redundant - and harmful: on-demand recursion demands every NESTED gitlink in the newly imported history from the nested remote, and one commit that remote no longer has fails the fetch. git then blames the OUTER submodule (`Fetched in submodule path 'X', but it did not contain <sha>. Direct fetching of that commit failed.`) because it checks the fetch's exit status before checking whether the object arrived. Pooled slots are where this bites, since a slot's submodule remote is the parent's module store, which advertises no `refs/heads/*` - so the direct-by-sha fallback is the fetch that imports everything, and its recursion has nothing behind it. Without this, every slot jams on the next commit that moves that gitlink
- That submodule reset widens what `return --force` destroys: before it, `--force` discarded superproject working-tree changes only (checkout/reset/clean do not recurse), and now it also discards tracked modifications and untracked files INSIDE submodules. `--force` is the only silent-destruction path, so its flag help and the README flag table must keep saying so; plain `return` still prompts and aborts on a non-interactive EOF, and `Acquire` still skips dirty slots
